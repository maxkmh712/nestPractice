...
let ContentService = class ContentService {
    constructor(contentRepository, contentError) {
        this.contentRepository = contentRepository;
        this.contentError = contentError;
    }
    async findContents() {
        try {
            throw Error(content_error_1.CONTENT_ERROR.CONTENT_NOT_FOUND);
            return await this.contentRepository.find();
        }
        catch (error) {
            console.log(error);
            throw new common_1.BadRequestException(this.contentError.errorHandler(error.message));
        }
    }
};
